...
export function useQuizEngine() {
  ...
  const masteryPercent = computed(() =>
    Math.round(
      Object.values(mastery.value).reduce((sum, x) => sum + x.score, 0) /
        concepts.length,
    ),
  );
  ...
  const sessionTarget = computed(
    () => ({ learn: 25, exam: 50, weak: 20, grind: 0 })[mode.value],
  );
  const questionPosition = computed(() =>
    mode.value === "exam"
      ? examIndex.value + 1
      : sessionRecords.value.length +
        (recoveryRecordId.value ||
        sessionRecords.value.some((r) => r.question.id === current.value?.id)
          ? 0
          : 1),
  );
  const progressText = computed(() =>
    sessionTarget.value
      ? `${Math.min(questionPosition.value, sessionTarget.value)} / ${sessionTarget.value}`
      : `${sessionRecords.value.length} done`,
  );
  const reportRecords = computed(
    () => activeReport.value?.records || sessionRecords.value,
  );
  const auditedRecords = computed(() =>
    reportRecords.value.filter((record) => !record.excludedFromAudit),
  );
  const firstTryCorrect = computed(
    () => auditedRecords.value.filter((r) => r.firstCorrect).length,
  );
  const recoveredCount = computed(
    () =>
      auditedRecords.value.filter((r) => !r.firstCorrect && r.solved).length,
  );
  const sessionAccuracy = computed(() =>
    auditedRecords.value.length
      ? Math.round((firstTryCorrect.value / auditedRecords.value.length) * 100)
      : 0,
  );
  const sessionAccuracyLabel = computed(() =>
    auditedRecords.value.length ? String(sessionAccuracy.value) : "—",
  );
  const knownCount = computed(
    () =>
      reportRecords.value.filter((record) => record.disposition === "known")
        .length,
  );
  const skippedCount = computed(
    () =>
      reportRecords.value.filter((record) => record.disposition === "skipped")
        .length,
  );
  const earnedPerfectionist = computed(
    () =>
      reportRecords.value.length > 0 &&
      skippedCount.value === 0 &&
      reportRecords.value.every((record) => record.firstCorrect),
  );
  const currentAttempts = computed(
    () =>
      sessionRecords.value.find(
        (r) => r.question.id === (recoveryRecordId.value || current.value?.id),
      )?.attempts || 0,
  );
  ...
  const correctDefinition = computed(() =>
    findGlossaryEntry(current.value?.answer),
  );
  const matchingElementNumbers = computed(() => {
    const query = elementQuery.value.trim().toLowerCase();
    if (!query)
      return new Set(periodicTable.map((element) => element.atomicNumber));
    return new Set(
      periodicTable
        .filter(
          (element) =>
            element.name.toLowerCase().includes(query) ||
            element.symbol.toLowerCase() === query ||
  ...
  const reportWeakPoints = computed(() => {
    ...
    auditedRecords.value
      .filter((record) => !record.firstCorrect)
      .forEach((record) =>
        counts.set(
          record.question.topic,
          (counts.get(record.question.topic) || 0) + 1,
        ),
      );
  ...
  const historicalAccuracy = (report: SessionReport) => {
    const audited = report.records.filter(
      (record) => !record.excludedFromAudit,
    );
    return audited.length
      ? `${Math.round((audited.filter((record) => record.firstCorrect).length / audited.length) * 100)}%`
      : "—";
  };

  function generateFor(id: ConceptId) {
  ...
  function weightedConcept(): ConceptId {
    ...
    if (mode.value === "weak") {
      const weak = concepts.filter(
        ([id]) => mastery.value[id].score < 67 || mastery.value[id].wrong > 0,
      );
    ...
    const pool = candidates.flatMap((c) =>
      Array(Math.max(1, 6 - Math.floor(mastery.value[c[0]].score / 20))).fill(
        c[0],
      ),
    );
  ...
  function nextQuestion() {
    ...
    let tries = 0;
    do {
      q = generateFor(weightedConcept());
      tries++;
    } while (recentIds.value.includes(q.conceptId) && tries < 5);
    current.value = q;
  ...
  function check(q: Question, value: string) {
    if (typeof q.answer === "number") {
      const parsed = Number(
        value.replace(/×\s*10\^?/i, "e").replace(/\s/g, ""),
      );
      return (
        Number.isFinite(parsed) &&
        Math.abs(parsed - q.answer) <= (q.tolerance ?? 0.001)
      );
    }
  ...
  function submit() {
    ...
    let record = sessionRecords.value.find(
      (r) => r.question.id === (recoveryRecordId.value || current.value!.id),
    );
    ...
    if (!record) {
      record = {
        ...
        attempts: 0,
        firstCorrect: wasCorrect.value,
        solved: false,
        firstResponse: response.value,
        retryResponses: [],
  ...
  function markKnown() {
    ...
    const record: AttemptRecord = {
      ...
      firstCorrect: true,
      solved: true,
      firstResponse: "Marked “I know this”",
      retryResponses: [],
      excludedFromAudit: true,
      disposition: "known",
      masteryBefore: m.score,
    };
    sessionRecords.value.push(record);
    stats.value.attempted++;
    stats.value.correct++;
  ...
  function reconsiderKnown(useFreshVariant = false) {
    ...
    const recordIndex = sessionRecords.value.findIndex(
      (record) => record.question.id === current.value!.id,
    );
    ...
    if (useFreshVariant) {
      ...
      const candidates = Array.from({ length: 10 }, () =>
        generateFor(original.conceptId as ConceptId),
      );
      current.value =
        candidates.find((candidate) => candidate.prompt !== original.prompt) ||
        candidates[0]!;
    }
    response.value = "";
  ...
  function retryQuestion() {
    ...
    const candidates = Array.from({ length: 12 }, () =>
      generateFor(previousQuestion.conceptId as ConceptId),
    );
    const replacement = (candidates.find(
      (candidate) =>
        candidate.prompt !== previousQuestion.prompt &&
        normalize(String(candidate.answer)) !==
          normalize(String(previousQuestion.answer)) &&
        normalize(String(candidate.answer)) !== normalize(previousResponse),
    ) ||
      candidates.find(
        (candidate) => candidate.prompt !== previousQuestion.prompt,
      ) ||
      candidates[0])!;
    ...
    if (replacement.choices) {
      const distractorPool = candidates
        ...
        .filter(
          (value) =>
            normalize(value) !== normalize(String(replacement.answer)) &&
            normalize(value) !== normalize(previousResponse),
        )
        .filter(
          (value, index, values) =>
            values.findIndex(
              (other) => normalize(other) === normalize(value),
            ) === index,
        );
      if (distractorPool.length >= 3)
        replacement.choices = choice(
  ...
  function advanceQuestion() {
    if (
      sessionTarget.value &&
      sessionRecords.value.length >= sessionTarget.value
    )
  ...
  function persist() {
    localStorage.setItem(
      "chem-grind-progress",
      JSON.stringify({ mastery: mastery.value, stats: stats.value }),
    );
  }
  function persistSession() {
    if (screen.value !== "quiz" || !current.value) return;
    localStorage.setItem(
      "chem-grind-active-session",
      JSON.stringify({
        mode: mode.value,
        current: current.value,
        examQuestions: examQuestions.value,
        examIndex: examIndex.value,
        sessionRecords: sessionRecords.value,
  ...
  function resumeSession() {
    ...
    try {
      ...
      sessionRecords.value = (data.sessionRecords || []).map(
        (record: Partial<AttemptRecord>) => ({
          ...record,
          firstResponse:
            record.firstResponse || "Not recorded in this earlier session",
  ...
  function handleKey(e: KeyboardEvent) {
    if (e.key === "Escape" && showTools.value) {
    ...
    if (e.key === "Enter" && screen.value === "quiz") {
      if (submitted.value)
        wasCorrect.value ? advanceQuestion() : retryQuestion();
      else submit();
    }
  }
  ...
  onMounted(() => {
    ...
    if (savedReports) {
      try {
        reportHistory.value = JSON.parse(savedReports);
      } catch {
        localStorage.removeItem("quiz-taker-reports-v1");
      }
    }
    resumeAvailable.value = Boolean(
      localStorage.getItem("chem-grind-active-session"),
    );
